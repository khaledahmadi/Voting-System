	...
	<fieldset>
		...
		$results = $db->prepare("SELECT * FROM candidates WHERE position= :a");
			...
			for($i=0; $rows = $results->fetch(); $i++){
				?>
			<img src="candidates/<?php echo $rows['image']; ?>"   class="image" />
				<option style="padding: 35px 50px 35px 80px; background:url('admin/candidates/<?php echo $rows['image']; ?>')  no-repeat scroll 5px 7px / 70px auto rgba(0, 0, 0, 0);" value="<?php echo $rows['name'] ?>"><?php echo $rows['name'] ?> - <?php echo $rows['party'] ?>- <?php echo $rows['course'] ?></option>
				<?php
			}
		?>
		</select>
