...
<div id="whole">
<?php
//if(isset($_GET['name']) AND  isset($_GET['position']) AND isset($_GET['party']) AND isset($_GET['numberofvotes']) AND isset($_GET['id'])){
...
$results = $db->prepare("SELECT * FROM votedetails WHERE candidate= :a");
	...
	for($i=0; $rows = $results->fetch(); $i++){
		$dddddd=$rows['voters'];
		$resultas = $db->prepare("SELECT * FROM students WHERE id= :j");
		$resultas->bindParam(':j', $dddddd);
		$resultas->execute();
		for($i=0; $rowas = $resultas->fetch(); $i++){
		?>
		<tr class="record">
			<td><?php echo $rowas['username']; ?></td>
